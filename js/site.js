...
function itemAddInfo( item ) {
  ...
  var mainElement = item.mainElement;
...
function galleryItemCreate(element) {
  var reflection;
  if ($(element).is('img')) {
      reflection = $($(element).reflect(mirrorOpts)).next()[0];
      $(reflection).css('margin-top', mirrorOpts.gap + 'px');
  } else if ($(element).is('a')) {
      reflection = $($(element).find('img').reflect(mirrorOpts)).next()[0];
      $(reflection).css('margin-top', mirrorOpts.gap + 'px');
  }

  // Set width for both img and a elements
  $(reflection).css('width', '100%');
  $(element).css('width', '100%');

  // reflect() wrapped the image or link in a container element
  var item = $(element).parent();

  // Save a reference to the main element (img or a)
  item.mainElement = element;
...
function loadMoreGallery(file) {
  ...
  $.get(file).done(function (data) {
      ...
      // Add images or links to the DOM
      $(data).filter('img, a').each(function () {
          if ($(this).is('img')) {
      ...
      // Wait until images or links are fully loaded...
      for (var i in items) {
